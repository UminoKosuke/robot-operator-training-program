...
[everyday-todo.md](./everyday-todo.md)で`uv run lefobot-find-cameras`した後、setup.shに確認したカメラIDを設定しているが、まれにcameraのindexが変わることがある（コマンドがエラーになる）ので、その際は`uv run lerobot-find-cameras`で改めてcameraのインデックスを確認。その後`setup.sh`に値を設定して、再度`source setup.sh`を実行。そして、`echo $FRONT_CAM_ID`で設定したカメラIDが表示されることを確認

## テレオペ確認

以下のコマンドを実行して、テレオペレーションができることを確認する。`--display_data=true`で実行し、カメラにラグが発生している場合は、ネットワークが正しく形成されていない、またはUSBの帯域が過多になっている可能性がある。
...
テレオペレーションできるようになったら、データ収集を開始する。以下のコマンドを実行する前に、環境のセットアップを行う。（口頭で説明します。）その後、実際に以下のコマンドでデータ収集を開始する。データセット（リポジトリが構成）されていない場合は`--resume=false`にして一度データを収集し、その後は`--resume=true`にしてデータを収集する。なお、1データ収集ごとに再度環境のセットアップを行う。
...
1. [./init-pose.md](./init-pose.md)に従って初期姿勢をとる。
2. 以下の内容に従ってタスクを実行する。
   - つかむcubeの上まで腕を伸ばす
   - グリッパーを解放する
   - 腕を下ろし、cubeがつかめる高さに調節する
   - グリッパーを閉じて、cubeをつかむ
   - 腕を上げる
   - 腕をトレイの中心方向に移動させる
   - 腕を下ろす
   - グリッパーを解放する
   - 腕を上げる
3. [./init-pose.md](./final-pose.md)に従って完了姿勢をとる。[./final-pose.md]
